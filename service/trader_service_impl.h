...
public:
  TraderServiceImpl(const rapidjson::Document &doc, TraderCallback *callback);
  ...
  virtual void cancelOrder(int32_t order_ref, const std::string &instru);
  ...
  virtual void queryPosition(const std::string &instru);

  virtual void queryAccount(const std::string &currency_id);

  virtual void queryInvestor();

  virtual void queryTradingCode(const std::string &exchange);

  virtual void queryInstruMarginRate(const std::string &instru);

  virtual void queryInstruCommissionRate(const std::string &instru);

  virtual void queryExchange(const std::string &exchange);

  virtual void queryProduct(const std::string &product_id);
  ...
  virtual void queryDepthMarketData(const std::string &instru);

  virtual void queryExchangeMarginRate(const std::string &instru);

  virtual void queryExchangeMarginRateAdjust(const std::string &instru);

  void reqAuthenticate();

  void login();

  void rspLogin(const CThostFtdcRspUserLoginField *rsp);
  ...
  TraderCallback *callback() { return callback_; }

  void notify() { cond_->notifyAll(); }

protected:
  void settlementInfoConfirm();

  std::shared_ptr<CThostFtdcInputOrderField> reqOrderMessage(
      const std::string &instru, double price, int volume,
  ...
  int32_t orderGo(std::shared_ptr<CThostFtdcInputOrderField> req);

  int reqID() { return ++request_id_; }
